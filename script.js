...
RID.Assets = (function () {
  ...
  function preload(onProgress, onDone) {
    ...
    for (k in manifest.img)   keys.push({ kind: 'img',   key: k, path: manifest.img[k] });
    for (k in manifest.audio) keys.push({ kind: 'audio', key: k, path: manifest.audio[k] });
